...
No package manager. Plain Python 3.11 repo (`.python-version`); deps pinned in `requirements.txt` (`numpy<2.0`, `torch==2.4.0`). Not installed as a package — always run scripts from repo root, since all imports (`from dlamp...`) assume CWD = repo root. Hydra runs with `chdir: False`.
...
- `src/dlamp/const.py` reads env vars: `DLAMP_EXP_CODE` (default `20250627`), `DLAMP_DATA_SOURCE` (default `OP_ERA5`), `DLAMP_DATA_PATH` (default `/wk2/yaochu/CASE_DATA/Pool/`).
...
- Raw per-hour NetCDF input lives in `DLAMP_DATA_PATH`; filename pattern depends on `DLAMP_DATA_SOURCE` (`src/dlamp/utils/file_util.py:gen_path`).
...
`src/dlamp/models/architectures/*_test.py` are `unittest`, run via `python -m dlamp.models.architectures.<name>_test`. They are stale: `pangu_model_test.py` opens a non-existent `config/model/dlamp_train.yaml` and crashes at import. Do not assume tests pass or match current configs.
