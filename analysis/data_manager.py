...
Usage:
    ...
    fc_f000 = data_manager.get_forecast_data(-1, DataType.T, Level.Hpa500)
...
class AnalysisDataManager:
    ...
    def __init__(self, cfg: DictConfig, prediction_results: Dict[str, Any]):
        ...
        logger.info("AnalysisDataManager initialized.")
    ...
    def _get_wind_components(
        ...
        u: np.ndarray = self.get_forecast_data(forecast_step, DataType.U, level)
        v: np.ndarray = self.get_forecast_data(forecast_step, DataType.V, level)
        return u, v
    ...
    def _get_gt_wind_components(
        ...
        u: np.ndarray = self.get_ground_truth_data(time, DataType.U, level)
        v: np.ndarray = self.get_ground_truth_data(time, DataType.V, level)
        return u, v
    ...
    def get_column_max_qw(
        ...
        for level in qw_levels:
            data: np.ndarray
            if is_gt:
                data = self.get_ground_truth_data(time, DataType.Qw, level)
            else:
                data = self.get_forecast_data(
                    forecast_step, DataType.Qw, level
                )
